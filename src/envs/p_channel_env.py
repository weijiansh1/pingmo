...
class RollQualityEnv(gym.Env[np.ndarray, np.ndarray]):
    ...
    def __init__(self, plants: list[PlantRecord], horizon_steps: int = 250, action_limit: float = 1.0, correction_ratio: float = 0.3, pilot_signal: str = "sine", pilot_force_scale_n: float = 22.0, normalized_rate_limit_s_inv: float = 4.0, history_steps: int = 32) -> None:
        if not plants:
        ...
        if pilot_force_scale_n <= 0 or normalized_rate_limit_s_inv <= 0 or history_steps <= 0:
            raise ValueError("force scale, rate limit, and history steps must be positive")
        ...
        self.observation_space = gym.spaces.Box(-np.inf, np.inf, (5 + history_steps * 4 + 8,), np.float32)
    ...
    def _append_history(self) -> None:
        ...
        self._history[-1] = np.array([self._f_pilot, self._p, self._p_ref, self._previous_action * self.correction_ratio * self.pilot_force_scale_n], dtype=np.float32)

    def _observation(self) -> np.ndarray:
        error = self._p - self._p_ref
        current = np.array([self._f_pilot, self._p, self._p_ref, error, self._previous_action * self.correction_ratio * self.pilot_force_scale_n], dtype=np.float32)
    ...
    def reset(self, *, seed: int | None = None, options: dict | None = None) -> tuple[np.ndarray, dict]:
        ...
        self._episode_step, self._previous_action = 0, 0.0
    ...
    def step(self, action: np.ndarray) -> tuple[np.ndarray, float, bool, bool, dict]:
        ...
        normalized_action = float(np.clip(requested_action, previous_action - max_action_increment, previous_action + max_action_increment))
        delta = normalized_action * self.correction_ratio * self.pilot_force_scale_n
        previous_delta = previous_action * self.correction_ratio * self.pilot_force_scale_n
        ...
        reward = roll_quality_reward((self._p - self._p_ref) / p_scale, normalized_action, normalized_action - previous_action)
        self._diagnostic_count += 1
        self._saturation_count += int(abs(normalized_action) >= self.action_limit - 1e-9)
