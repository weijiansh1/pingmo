...
def test_doublet_profile_reverses_then_releases() -> None:
    ...
    samples = profile.samples(plant_dt_s=0.02, duration_s=4.0, nominal_force_n=22.0)
...
def test_command_profile_samples_are_deterministic_and_force_scaled() -> None:
    profile = CommandProfile("sine-0.50hz", "sine", amplitude=0.5, frequency_hz=0.5, onset_s=0.0)

    first = profile.samples(plant_dt_s=0.02, duration_s=2.0, nominal_force_n=22.0)
    second = profile.samples(plant_dt_s=0.02, duration_s=2.0, nominal_force_n=22.0)
...
def test_default_command_suite_contains_all_required_families() -> None:
    ...
    assert kinds == {"step", "pulse", "doublet", "square", "sine", "chirp", "staircase", "piecewise"}
...
def test_default_command_cannot_silently_change_its_force_history() -> None:
    ...
    with pytest.raises(ValueError, match="is defined for"):
        profile.samples(plant_dt_s=0.001, duration_s=5.0, nominal_force_n=22.0)
