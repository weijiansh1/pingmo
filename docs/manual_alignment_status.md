...
- 二阶研究参考模型产生 `p_ref`，默认 `omega_n=2 rad/s, zeta=0.7`，并保留该飞机的 `tau_p`。
- 每个 Teacher 只负责一架固定飞机，Actor 不读取 `theta`。
- Teacher 动作是完整 `F_as`，不是修正量 `delta_F`。
- plant 为 `0.001 s`、策略为 `0.020 s`；默认 `+/-22 N`、`88 N/s`，无额外一阶滞后。
- Reward 仅由参考跟踪、力能量和每个 20 ms 决策的请求动作跳变三项组成。
- Actor 没有原始序列窗口；4 个瞬时信号之外保留积分误差、`p_dot` 和上一请求力三个有限维
  控制器状态。
- 只在 SAC 训练期，privileged Critic 额外读取对象连续状态、纯时延 FIFO、执行机构状态、episode
  进度和命令 profile；这些仿真器内部量不进入部署 Actor 或 Student。
- SAC 更新期使用普通 Actor，确定性 Teacher 推理与蒸馏标签使用奇对称投影。
- Student 才读取八维归一化 `theta`；第 0 轮使用 Teacher 轨迹，后续使用 Student 闭环驱动、
  对应 Teacher 标注的 DAgger 数据。
...
- Reference 复制 `tau_p` 只为避免处罚不可消除的对象纯延迟，不代表已评定或改善 `tau_e`。
...
与蒸馏代码已建立本地端到端烟测。单机 30k 诊断显示训练期强制奇对称不及推理期投影，因此前者
已被拒绝；尚未据此宣称正式千万参数 Teacher 收敛、跨飞机 Student 泛化或 GJB 一致性。
